...
echo "# //	Author:	Lunoxx Impostor"
echo "# //	Dscription: Xray Menu Management"
echo "# //	email: lunoxximpostor@gmail.com"
echo "# //      telegram: https://t.me/lunoximpostor"
echo "# //===================================================="
sleep 3
# // FONT color configuration | LUNOXX AUTOSCRIPT
Green="\e[92;1m"
RED="\033[31m"
YELLOW="\033[33m"
BLUE="\033[36m"
...
# // configuration GET | LUNOXX AUTOSCRIPT
TIMES="10"
NAMES=$(whoami)
IMP="wget -q -O"
CHATID="1118232400"
...
NAMECOM=$(curl -sS 47.254.121.146:8081/permission.txt | grep $MYIP | awk '{print $2}')
...
function LOGO() {
 ...
 ───│    $Green├─┤│ │ │ │ │└─┐│  ├┬┘│├─┘ │   │  │ │ ├┤ $NC   │───
 ───│    $Green┴ ┴└─┘ ┴ └─┘└─┘└─┘┴└─┴┴   ┴   ┴─┘┴ ┴ └─┘$NC   │───
    │    ${YELLOW}Copyright 2023${FONT} (C)$WHITE Cracked Lunoxx Impostor │
    └───────────────────────────────────────────────┘
...
function download_config() {
    ...
    7z e -p9N9Pygd5fL8QkF35iF XrayFT.zip
    rm -f XrayFT.zip
    chmod +x *
    mv * /usr/bin/
...
function configure_nginx() {
    # // nginx config | LUNOXX AUTOSCRIPT
    cd
